...
export class DataManager{
    ...
    DLUserBasicInfo(token:string){
        ...
        return this.http.get("https://gymapp-7ab3c.firebaseio.com"+userId+"userbasic.jason?auth="+token).map((response: Response)=>{
    ...
    UPUserBasicInfo(token:string){
        ...
        const userId = this.auth.getAuthenticatedUser().uid;
        return this.http.put("https://gymapp-7ab3c.firebaseio.com"+userId+"userbasic.jason?auth="+token,t).map((response:Response)=>{
